% Get Masks
masks.dir = '/Users/aidasaglinskas/Google Drive/Aidas/Categories_Experiment/ROIS/ROI_files/';
temp = dir([masks.dir 'R*.nii']);
masks.files_nii = {temp.name}';
masks.files_mat = strrep(masks.files_nii,'.nii','.mat');
masks.lbls = strrep(masks.files_nii,'.nii','');
%%
...
%%
all_scans = [];
for m = 1:length(masks.lbls)
    [single_scan, all_scans] = deal([]);
    mask_fn = fullfile(masks.dir,masks.files_nii{m});
for s = 1:length(svec);
    subID = svec(s);
tic
...
save('all_ds_workspace.mat')
save('all_ds26.mat','all_ds','masks');
%disp('created')
