...
CC_DS = cosmo_slice(all_scans_raw,all_scans_raw.sa.subID==svec(s_ind));
CC_DS = cosmo_slice(CC_DS,CC_DS.sa.cond_index==18);
CC_DS = cosmo_slice(CC_DS,CC_DS.sa.b_ind==b_ind);

samples(:,:,b_ind) = this_DS.samples;
CCsamples(:,:,b_ind) = CC_DS.samples;
end

samples = mean(samples,3);
CCsamples = mean(CCsamples,3);

this_DS.samples = samples - CCsamples;
